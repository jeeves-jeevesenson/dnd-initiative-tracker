...
class MonsterIndexTempTests(unittest.TestCase):
    def test_load_monsters_index_includes_temp_relative_paths(self):
        import tempfile
        from pathlib import Path
        import yaml

        class Harness:
            _load_monsters_index = tracker_mod.InitiativeTracker._load_monsters_index
            _find_monster_spec_by_slug = tracker_mod.InitiativeTracker._find_monster_spec_by_slug
            _monster_int_from_value = tracker_mod.InitiativeTracker._monster_int_from_value
            _monsters_dir_path = tracker_mod.InitiativeTracker._monsters_dir_path
            _load_monster_details = tracker_mod.InitiativeTracker._load_monster_details
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (temp / "temp-wolf-test.yaml").write_text(
                yaml.safe_dump(
                    {
                        "name": "Temp Wolf",
                        "type": "beast",
            ...
            h = Harness()
            with mock.patch("dnd_initative_tracker._app_base_dir", return_value=base_dir):
                h._load_monsters_index()
                spec = h._find_monster_spec_by_slug("temp-wolf-test")
            self.assertIsNotNone(spec)
            self.assertEqual(spec.filename, "temp/temp-wolf-test.yaml")


class CustomSummonYamlTests(unittest.TestCase):
    def test_write_temp_monster_yaml_creates_file_and_slug(self):
        import tempfile
        from pathlib import Path

        class Harness:
            _write_temp_monster_yaml = tracker_mod.InitiativeTracker._write_temp_monster_yaml
            _sanitize_temp_monster_slug = staticmethod(tracker_mod.InitiativeTracker._sanitize_temp_monster_slug)
            _monsters_dir_path = tracker_mod.InitiativeTracker._monsters_dir_path

        with tempfile.TemporaryDirectory() as td:
            base_dir = Path(td)
            (base_dir / "Monsters").mkdir(parents=True, exist_ok=True)
            h = Harness()
            with mock.patch("dnd_initative_tracker._app_base_dir", return_value=base_dir):
                slug, path = h._write_temp_monster_yaml(
                    {
                        "name": "Summoned Beast",
                        "hp": 30,
                        "speed": {"speed": 30, "swim": 10, "fly": 0, "burrow": 0, "climb": 5},
                        "abilities": {"str": 14, "dex": 12, "con": 13, "int": 3, "wis": 10, "cha": 6},
                    }
                )
            self.assertTrue(slug)
            self.assertTrue(path and path.exists())
            self.assertIn("Monsters/temp", str(path))
