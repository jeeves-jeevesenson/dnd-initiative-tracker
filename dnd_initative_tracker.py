    ...
    def _lan_apply_action(self, msg: Dict[str, Any]) -> None:
        ...
        if typ == "cast_aoe":
            ...
            try:
                ...
                return
            def _try_convert(value: Any, conversion_func: Callable[[Any], Any]) -> Optional[Union[int, float]]:
                try:
                    return conversion_func(value)
                except Exception:
                    return None
            angle_deg = _try_convert(to.get("angle_deg"), float)
            ax = _try_convert(to.get("ax"), float)
            ay = _try_convert(to.get("ay"), float)
            spread_deg = _try_convert(to.get("spread_deg"), float)
            mw = getattr(self, "_map_window", None)
            map_ready = mw is not None and mw.winfo_exists()
            ...
            cid_int = _try_convert(cid, int)
            claimed_int = _try_convert(claimed, int)
            owner_cid_int = _try_convert(owner_cid, int)
            anchor_cid_int = _try_convert(anchor_cid, int)
            active_cid = self.current_cid
            active_cid_int = _try_convert(active_cid, int)
