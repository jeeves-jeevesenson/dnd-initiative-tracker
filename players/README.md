...
vitals:
  ...
  speed:
    Normal: 30 ft.                # Walking speed
    Climb: 15 ft.                 # Climbing speed (0 ft. if none)
    Fly: 0 ft.                    # Flying speed (0 ft. if none)
    Swim: 30 ft.                  # Swimming speed (0 ft. if none)
  initiative:
...
- **speed**: Movement speeds per round (monster-style map with `ft.` units)
  - **Normal**: Standard walking speed (typically 30 ft. for most races, 25 ft. for dwarves/small races)
...
- For `format_version: 2` files, `vitals.speed` is used to populate the roster movement speeds using the monster-style keys (`Normal`, `Climb`, `Fly`, `Swim`, `Burrow`). Values preserve their formatting (e.g., `30 ft.`).
